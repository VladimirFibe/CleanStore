...
class OrdersMemStore: OrdersStoreProtocol, OrdersStoreUtilityProtocol {
    ...
    static var billingAddress = Address(
        ...
        state: "CA",
        zip: "95014")
    static var shipmentAddress = Address(
    ...
    static var orders = [
        ...
        Order(
            ...
            lastName: "Battery",
            phone: "222-222-2222",
            email: "bob.battery@clean-swift.com",
            billingAddress: billingAddress,
            paymentMethod: paymentMethod,
            shipmentAddress: shipmentAddress,
            shipmentMethod: shipmentMethod,
            id: "def456",
            date: Date(),
            total: NSDecimalNumber(string: "4.56")
        )
    ]
    ...
    func updateOrder(
        orderToUpdate: Order,
        completionHandler: @escaping OrdersStoreUpdateOrderCompletionHandler
    ) {
        if let index = indexOfOrderWithID(id: orderToUpdate.id) {
            type(of: self).orders[index] = orderToUpdate
            let order = type(of: self).orders[index]
    ...
    func deleteOrder(
        id: String,
        completionHandler: @escaping OrdersStoreDeleteOrderCompletionHandler
    ) {
        if let index = indexOfOrderWithID(id: id) {
            let order = type(of: self).orders.remove(at: index)
            completionHandler(OrdersStoreResult.Success(result: order))
